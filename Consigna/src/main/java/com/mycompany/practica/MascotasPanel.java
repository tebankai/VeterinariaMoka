...
public class MascotasPanel extends JPanel {
    ...
    String rankingStr = txtranking.getText();

    // Validar que los campos no estén vacíos
    if (nombre.isEmpty() || dueno.isEmpty() || observaciones.isEmpty() || rankingStr.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Por favor complete todos los campos.", "Error", JOptionPane.ERROR_MESSAGE);
        return;
    }

    // Validar que el ranking sea un número entero
    int ranking;
    try {
        ranking = Integer.parseInt(rankingStr);  // Convertir el ranking a entero
    } catch (NumberFormatException ex) {
        JOptionPane.showMessageDialog(this, "El ranking debe ser un número entero.", "Error", JOptionPane.ERROR_MESSAGE);
        return;
    }

    // Verificar si ya existe una mascota con el mismo nombre
    for (int i = 0; i < tableModel.getRowCount(); i++) {
        if (tableModel.getValueAt(i, 0).equals(nombre)) {  // 0 es la columna de "Nombre"
            JOptionPane.showMessageDialog(this, "Ya existe una mascota con ese nombre.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
    }
